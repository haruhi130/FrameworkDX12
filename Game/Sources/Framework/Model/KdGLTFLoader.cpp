...
// 　・座標：zに-1を乗算
//===================================================
std::shared_ptr<GLTFModel> LoadGLTFModel(std::string_view path)
{
#ifdef GLTF_DEBUG
	// コンソールウィンドウ表示
	if (AllocConsole()) {
	...
	for (UINT nodei = 0; nodei < destModel->Nodes.size(); nodei++)
	{
			...
			// 頂点バッファ
			{
				// 座標
				{
					...
					for (UINT vi = 0; vi < posGetter.GetAccessor()->count; vi++) {
						...
						ver.Position.x = posGetter.GetValue_Float(vi * 3 + 0);
						ver.Position.y = posGetter.GetValue_Float(vi * 3 + 1);
						ver.Position.z = posGetter.GetValue_Float(vi * 3 + 2) * -1;
					}
				}
				...
				if (model.skins.size() > 0)
				{
					...
					if (srcPrimitive.attributes.count("WEIGHTS_0") > 0)
					{
						...
						for (UINT vi = 0; vi < destPrimitive->Vertices.size(); vi++)
						{
							...
							skinWei[static_cast<std::array<float, 4Ui64>::size_type>(cnt) - 1] = 1.0f - totalW;
						}
					}
				}
			}
		...
		for (UINT pi = 0; pi < tempPrimitives.size(); pi++)
		{
			...
			if (prim->Vertices.size() >= 1) {
				size_t st = destNode->Mesh.Vertices.size();
			...
			if (prim->Faces.size() >= 1) {
				size_t st = destNode->Mesh.Faces.size();
