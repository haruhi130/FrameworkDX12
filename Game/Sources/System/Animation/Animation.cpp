...
bool AnimationData::Channel::InterpolateTranslations(Math::Vector3& result, float time)
{
	...
	// それ以外(中間の時間)なら、その時間の値を補間計算で求める
		...
		auto& prev = m_translations[static_cast<std::vector<AnimeKeyVector3, std::allocator<AnimeKeyVector3>>::size_type>(keyIdx) - 1];	// 前のキー
...
bool AnimationData::Channel::InterpolateRotations(Math::Quaternion& result, float time)
{
	...
	// それ以外(中間の時間)なら、その時間の値を補間計算で求める
		...
		auto& prev = m_rotations[static_cast<std::vector<AnimeKeyQuaternion, std::allocator<AnimeKeyQuaternion>>::size_type>(keyIdx) - 1];	// 前のキー
...
bool AnimationData::Channel::InterpolateScales(Math::Vector3& result, float time)
{
	...
	// それ以外(中間の時間)なら、その時間の値を補間計算で求める
		...
		auto& prev = m_scales[static_cast<std::vector<AnimeKeyVector3, std::allocator<AnimeKeyVector3>>::size_type>(keyIdx) - 1];	// 前のキー
